import { BufferAttribute, BufferGeometry, ClampToEdgeWrapping, Clock, DoubleSide, LinearFilter, Mesh, PerspectiveCamera, PlaneGeometry, Points, Scene, ShaderMaterial, TextureLoader, Vector2, Vector3, WebGLRenderer, } from 'three';
...
class FancyCursor {
    ...
    constructor(userConfig) {
        ...
        const onTextureLoaded = () => {
            if (!t1 || !t2 || !t4)
                return;
            const linef2 = linef.replace('float t = 2.5 - pow(vFx.x, 0.5) * 2.7;', 'float t = 2.5 - pow(vFx.x, 0.5) * ' +
